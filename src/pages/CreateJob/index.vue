<template>
  <div>
    ...
    <el-card class="card-style" shadow="hover" :body-style="{ padding: '0px' }"
      ...
      <Transition appear enter-active-class="animate__animated animate__fadeInLeft"
        leave-active-class="animate__animated animate__fadeOutRight">
        <el-form ref="jobForm" label-position="right" label-width="100px" style="min-width: 1000px; padding: 0px"
          :model="vcJob" :rules="vcJobRules">
          <el-row>
            <el-col :span="11">
              <el-form-item label="作业中文名" prop="vcJobCnName" style="margin-left: 50px">
                <el-col>
                  <el-input size="small" placeholder="请输入作业中文名" style="width: 340px; margin-left: 0px"
            ...
            <el-col :span="2">
              <el-button type="primary" size="small" @click="jobDialogVisible = true"
                style="margin-top: 4px">高级配置</el-button>
            </el-col>
          </el-row>
          <el-dialog title="作业高级配置" :visible.sync="jobDialogVisible" width="800px">
            <el-row>
              <el-col :span="11">
                <el-form-item label="作业优先级" prop="priorityClassName">
                  <el-col>
                    <el-select size="small" placeholder="请选择作业的优先级" v-model="vcJob.priorityClassName"
                      style="width: 250px">
              ...
              <el-col :span="11">
                <el-form-item label="所属集群">
                  <el-col>
                    <el-select v-model="vcJob.queueName" size="small" style="width: 250px">
            ...
            <el-row>
              ...
              <el-col :span="11">
                <el-form-item label="重启次数" prop="maxRetry">
                  <el-col>
                    <el-input-number size="small" style="width: 250px" :min="0" :max="100"
                      v-model="vcJob.maxRetry"></el-input-number>
          ...
          <el-row>
            ...
            <el-col :span="1">
              <el-button type="primary" size="small" @click="getImage">添加</el-button>
            </el-col>
          </el-row>

          <el-table :cell-style="{ padding: 5 + 'px' }" style="
              width: 100%;
              margin-top: 10px;
              margin-left: 10px;
              font-size: 16px;
            " :data="tasks" :border="true" :row-style="{ height: 30 + 'px' }" max-height="400px">
            <el-table-column prop="taskCnName" label="任务中文名称"></el-table-column>
            <el-table-column prop="taskName" label="任务英文名称"></el-table-column>
            <!-- <el-table-column
              prop="podReplicas"
              label="实例个数"
            ></el-table-column>
            <el-table-column
              ...
              label="实例最小个数"
            ></el-table-column> -->
            <el-table-column prop="requestCpu" label="CPU核心数(核)"></el-table-column>
            <el-table-column prop="requestMemory" label="内存使用量(GB)"></el-table-column>
            <el-table-column label="操作" width="160">
              <template slot-scope="scope">
                <el-button size="mini" type="danger" @click="deleteTask(scope.$index)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-form-item style="width: 100%">
            <el-row>
              <el-col :span="2" :offset="22"><el-button type="primary" size="small"
                  @click="createJob">创建</el-button></el-col>
              <!-- <el-col :span="2"
                ><el-button
                  type="primary"
                  size="small"
    ...
    <el-dialog title="新建任务" :visible.sync="dialogFormVisible" width="1000px">
      <el-form ref="taskForm" :rules="taskRules" :model="taskInfo" label-position="right" label-width="70px"
        style="padding: 0px; min-width: 900px">
        <el-row>
          <el-col :span="12">
            <el-form-item label="任务英文名" :label-width="formLabelWidth" prop="taskName">
              <el-input autocomplete="off" v-model="taskInfo.taskName" placeholder="请输入任务英文名"
                style="width: 340px"></el-input> </el-form-item></el-col>
          <el-col :span="12">
            <el-form-item label="任务中文名" :label-width="formLabelWidth" prop="taskCnName">
              <el-input autocomplete="off" v-model="taskInfo.taskCnName" placeholder="请输入任务中文名"
                style="width: 340px"></el-input> </el-form-item></el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="7">
            <el-form-item label="选择镜像" :label-width="formLabelWidth" prop="imagePrefix">
              <el-select placeholder="请选择镜像" style="width: 150px" v-model="taskInfo.imagePrefix" @change="changeImage">
                <el-option v-for="(item, index) in sortedImageList" :key="index" :label="imageName(item)"
                  :value="item.imageName"></el-option>
              </el-select> </el-form-item></el-col>
          <el-col :span="8">
            <el-form-item label="选择版本" :label-width="formLabelWidth" prop="imageVersion">
              <el-select placeholder="请选择镜像版本" style="width: 180px" v-model="taskInfo.imageVersion">
                <el-option v-for="(item, index) in imageVersionList" :key="index" :label="item.imageTag"
                  :value="item.imageTag"></el-option>
        ...
        <el-form-item label="GPU算力" :label-width="formLabelWidth" style="width: 900px">
          <el-row style="margin-top: 0px">
            <el-col :span="20">
              <el-input-number style="width: 100%" v-model="taskInfo.gpuCalPower" v-if="taskInfo.gpuCalPower < 100"
                :min="0" :step="25" :max="100"
                @change="taskInfo.gpuVideoMem = taskInfo.gpuCalPower * 0.88"></el-input-number>
              <el-input-number style="width: 100%" v-model="taskInfo.gpuCalPower" v-if="taskInfo.gpuCalPower >= 100"
                :min="0" :step="100" :max="300"
                @change="taskInfo.gpuVideoMem = taskInfo.gpuCalPower * 0.88"></el-input-number>
            </el-col>
            <el-col :span="3">
            ...
            <el-col :span="1">
              <el-tooltip class="item" effect="dark" placement="top">
                ...
                <i class="el-icon-question" style="color: #409eff; margin-left: 10px; cursor: pointer"></i>
              </el-tooltip></el-col>
          </el-row>
        </el-form-item>
        <el-form-item label="GPU显存" :label-width="formLabelWidth" style="width: 900px">
          <el-row style="margin-top: 0px">
            <el-col :span="20">
              <el-slider :max="Number(88)" style="width: 100%" v-model="taskInfo.gpuVideoMem"></el-slider>
            </el-col>
            <el-col :span="3">
              <span style="font-size: 16px; margin-left: 10px">{{ Number(taskInfo.gpuVideoMem) * 0.25 }}GB</span>
            </el-col>
            <el-col :span="1">
              <el-tooltip class="item" effect="dark" placement="top">
                <div slot="content">
                ...
                <i class="el-icon-question" style="color: #409eff; margin-left: 10px; cursor: pointer"></i>
              </el-tooltip></el-col> </el-row></el-form-item>
        <!-- <el-form-item
          label="选择仓库"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="故障响应" :label-width="formLabelWidth">
              <el-select placeholder="请选择当前任务状态" style="width: 340px" v-model="taskInfo.policiesEvent"
                @change="changeTaskpoliciesEvent" @clear="taskInfo.policiesAction = ''" clearable>
          ...
          <el-col :span="12">
            <el-form-item label="响应方式" :label-width="formLabelWidth">
              <el-select placeholder="请选择变更任务状态" style="width: 340px" v-model="taskInfo.policiesAction" clearable
                @change="changeTaskpoliciesAction" @clear="taskInfo.policiesEvent = ''">
        ...
        <el-form-item :label-width="formLabelWidth">
          <el-button slot="label" size="small" type="primary" @click="show = !show"
            style="cursor: pointer">高级配置</el-button>
          ...
          <el-tabs style="width: 820px" v-if="show">
            ...
            <el-tab-pane label="存储变量">
              <el-card :body-style="{ padding: '0px' }">
                ...
                <el-form :model="mount" :rules="mountRules" style="padding: 0px" label-position="right"
                  label-width="136px" ref="storageForm">
                  <el-row>
                  ...
                  <el-dialog title="目录挂载" :visible.sync="childDialogTableVisible" append-to-body width="600px">
                    <el-row>
                      <el-col :span="12">
                        <el-input size="small" placeholder="搜索" v-model="dataName"></el-input>
                      </el-col>
                      <el-col :span="8" :offset="4">
                        <el-button-group>
                          <el-button :type="type == 1 ? 'success' : 'primary'" size="small"
                            @click="getDataSet(1)">模型数据</el-button>
                          <el-button @click="getDataSet(2)" :type="type != 1 ? 'success' : 'primary'"
                            size="small">模型结果</el-button>
                    ...
                    <div slot="footer" class="dialog-footer">
                      <el-button @click="childDialogTableVisible = false">取 消</el-button>
                      <el-button type="primary" @click="childDialogTableVisible = false">确 定</el-button>
                    </div>
                  </el-dialog>
                  <el-row style="margin-top: 10px">
                    <el-form-item label="容器内路径" prop="mountPath">
                      <el-col :span="24" :offset="1" style="margin-left: 0px">
                        <el-input size="small" style="width: 440px" v-model="mount.mountPath">
                        </el-input>
                      </el-col>
                    </el-form-item>
                  </el-row>
                  <el-form-item>
                    <el-button type="primary" size="small" style="margin-top: 15px"
                      @click="addMountData()">添加</el-button></el-form-item>
                </el-form>
                <el-row>
                  <h3 style="margin-left: 10px">
                    <span style="color: #409eff">|</span>&nbsp;管理变量
                ...
                <el-row>
                  <div style="padding: 10px">
                    <el-table :data="mountData" :row-style="{ height: 30 + 'px' }" max-height="200">
                      <el-table-column label="路径地址" prop="hostPath" show-overflow-tooltip>
                        <template slot-scope="scope">
                          <el-input size="mini" v-model="scope.row.hostPath" v-if="isShow[scope.$index]"
                      ...
                      <el-table-column label="容器内路径" prop="mountPath" show-overflow-tooltip>
                        <template slot-scope="scope">
                          <el-input size="mini" v-model="scope.row.mountPath" v-if="isShow[scope.$index]"
                            @blur="updateIsShow(scope.$index)">
                          </el-input>
                          <span v-if="!isShow[scope.$index]">{{
                            scope.row.mountPath
                      ...
                      <el-table-column label="操作">
                        <template slot-scope="scope">
                          <el-button v-if="!isShow[scope.$index]" type="primary" size="mini" style="margin-buttom: 3px"
                            @click="updateIsShow(scope.$index)">修改</el-button>
                          <!-- <el-button
                            ...
                            >保存</el-button
                          > -->
                          <el-button type="danger" size="mini" style="margin-buttom: 3px"
                            @click="deleteMountData(scope.$index)">删除</el-button>
                        </template>
                      </el-table-column>
                    </el-table>
                  </div>
        ...
        <el-form-item label="备注" :label-width="formLabelWidth">
          <el-input type="textarea" placeholder="请输入内容" maxlength="30" show-word-limit style="width: 820px"
            v-model="taskInfo.description"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button @click="cancleTask()" style="margin-left: 725px">取 消</el-button>
